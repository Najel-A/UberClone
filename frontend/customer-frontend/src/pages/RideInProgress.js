...
const RideInProgress = () => {
    ...
    const pollRide = async () => {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_RIDE_SERVICE_URL}/api/rides/${selectedRide._id}`
        );
  ...
  const handleCompleteRide = async () => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_RIDE_SERVICE_URL}/api/rides/rideCompleted`,
        selectedRide
      );
  ...
  const handleDownloadBill = async () => {
    ...
    if (rideForBill.driverId) {
      try {
        const res = await axios.get(
          `${process.env.REACT_APP_DRIVER_SERVICE_URL}/api/drivers/${rideForBill.driverId}`
        );
        driverName = `${res.data.firstName || res.data.firstname || ""} ${
    ...
    const cardNumber =
      userProfile.creditCardDetails?.cardNumber || userProfile.cardNumber || "";
    ...
    doc.text(`${new Date(rideForBill.dateTime).toLocaleString()}`, 45, y);
    y += 8;
    doc.text(`Pickup Location:`, 15, y);
    doc.text(`${rideForBill.pickupLocation?.address || ""}`, 45, y);
    y += 8;
    doc.text(`Dropoff Location:`, 15, y);
    doc.text(`${rideForBill.dropoffLocation?.address || ""}`, 45, y);
    y += 8;
    doc.text(`Credit Card:`, 15, y);
    doc.text(`**** **** **** ${last4}`, 45, y);
    y += 8;
    doc.text(`Driver:`, 15, y);
    doc.text(`${driverName || rideForBill.driverId || "N/A"}`, 45, y);
    ...
    doc.setFontSize(12);
    let fareY = y + 8;
    doc.text("Base Fare:", 18, fareY);
    doc.text("$7.00", 70, fareY, { align: "right" });
    fareY += 7;
    doc.text("Distance:", 18, fareY);
    doc.text("$8.84", 70, fareY, { align: "right" });
    fareY += 7;
    doc.text("Time:", 18, fareY);
    doc.text("$9.00", 70, fareY, { align: "right" });
    fareY += 7;
    doc.text("Rounding Down:", 18, fareY);
    doc.text("($0.90)", 70, fareY, { align: "right" });
    fareY += 7;
    doc.text("Discount subtotal:", 18, fareY);
    doc.text("($0.90)", 70, fareY, { align: "right" });
    fareY += 10;
    doc.setFont("helvetica", "bold");
    doc.text("Total Fare:", 18, fareY);
    doc.text(`$${rideForBill.price?.toFixed(2) || "N/A"}`, 70, fareY, {
      align: "right",
    });
    ...
    doc.setFontSize(12);
    statsY += 8;
    doc.text("Distance:", 122, statsY);
    doc.text(
      `${rideForBill.distanceCovered?.toFixed(2) || "N/A"} miles`,
      170,
      statsY,
      { align: "right" }
    );
            ...
            <>
              ...
              <div className="ride-details">
                ...
                <div className="detail-item">
                  ...
                  <span className="value">
                    {distanceCovered.toFixed(2)} / {totalDistance.toFixed(2)}{" "}
                    miles
                  </span>
                </div>
              </div>
              ...
              {completeMsg && (
                <div
                  style={{
                    marginTop: 12,
                    color: completeMsg.includes("Failed") ? "red" : "green",
                  }}
                >
                  {completeMsg}
                </div>
