  ...
  useEffect(() => {
    const fetchRides = async () => {
      ...
      try {
        ...
        const ridesWithCustomerNames = await Promise.all(res.data.map(async (ride) => {
          console.log('Processing ride:', ride);
          if (ride.customerId) {
            try {
              console.log('Fetching customer data for ID:', ride.customerId);
              const customerRes = await axios.get(`http://localhost:3000/api/customers/${ride.customerId}`);
              console.log('Customer data received:', customerRes.data);
              const customerName = `${customerRes.data.firstName || ''} ${customerRes.data.lastName || ''}`.trim();
        ...
        console.log('Final rides data with customer names:', ridesWithCustomerNames);
